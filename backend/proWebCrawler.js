        ...
        if (method === 'prowebcrawler' && USE_PRO_CRAWLER) {
          ...
          const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Apify timeout')), 8000)
              ...
              // Enhanced variant selectors for better detection
              
              // 1. Most specific - selected options
              '[data-testid*="selected"][data-testid*="option"]',
              '.SelectedOption:not(:empty)',
              '.selected-option:not(:empty)',
              '[class*="selected"][class*="option"]:not(:empty)',
              
              // 2. Generic variant selectors
              '.variant-selected:not(:empty)',
              '.option-selected:not(:empty)',
              '[data-selected="true"]:not(:empty)',
              '.swatch.selected:not(:empty)',
              '.color-option.selected:not(:empty)',
              '.size-option.selected:not(:empty)',
              
              // 3. Dropdown selections
              '.a-dropdown-prompt:not(:empty)',
              '.dropdown-selection:not(:empty)',
              'select option[selected]:not(:empty)',
              
              // 4. Amazon specific
              '#variation_color_name .selection:not(:empty)',
              '#variation_size_name .selection:not(:empty)',
              '.a-button-selected .a-button-text:not(:empty)',
              
              // 5. Target specific
              '[data-test*="selected"]:not(:empty)',
              
              // 6. Walmart specific
              '[data-automation-id*="selected"]:not(:empty)',
              
              // 7. Wayfair specific
              '[data-testid="SelectedOption"]:not(:empty)',
              '.ProductDetailOptions .selected:not(:empty)',
              
              // 8. Generic button/swatch states
              if (element) {
                let variantText = element.textContent?.trim() || element.getAttribute('title')?.trim() || element.getAttribute('aria-label')?.trim();
                
                if (variantText && variantText.length > 1 && variantText.length < 50) {
                  // Clean up the variant text
                  variantText = variantText.replace(/[^\w\s\-]/g, '').trim();
                  
                  // Skip generic/unhelpful variants
                  if (!variantText.match(/^(select|choose|option|default|none|n\/a|null|undefined|click|tap)$/i)) {
            ...
            // If no variant found, try JSON data extraction
            if (!data.variant) {
                ...
                if (content) {
                  try {
                    ...
                    const variantMatches = [
                      /"selectedOption":\s*"([^"]{2,40})"/,
                      /"selectedVariant":\s*"([^"]{2,40})"/,
                      /"currentOption":\s*"([^"]{2,40})"/,
                      /"color":\s*"([^"]{2,30})"/,
                      /"size":\s*"([^"]{2,20})"/,
                      /"style":\s*"([^"]{2,30})"/
                    ];
                    
                    for (const pattern of variantMatches) {
                      const match = content.match(pattern);
                      if (match && match[1]) {
                        const variant = match[1].trim();
                        if (!variant.match(/^(select|choose|option|default|none|n\/a)$/i)) {
            ...
            if (!data.variant && data.name) {
              const titleVariantMatch = data.name.match(/\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s*[-,]\s*([A-Z][a-z0-9]+)\b/);
              if (titleVariantMatch) {
                const variant = titleVariantMatch[2];
                if (variant && variant.length > 1 && variant.length < 30) {
