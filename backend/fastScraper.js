...
async function scrapeProduct(url) {
  ...
  // Try ScrapingBee if Apify failed
  if (USE_SCRAPINGBEE && !productData) {
    try {
      ...
      console.log('   ❌ ScrapingBee extraction failed:', error.message);
    }
  }
  
  // Try UPCitemdb if we have a product name but missing dimensions
  ...
  // Use estimation if all methods failed
  if (!productData) {
    productData = {
