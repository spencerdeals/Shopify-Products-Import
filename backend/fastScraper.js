...
const { computePricing, computeFreight } = require('./utils/pricing');
...
async function scrapeProduct(url) {
  ...
  try {
    // Avoid double-calculation if already present
    if (!product.shipping_handling) {
      // Use the safe carton ft³ chosen by estimator
      const rawCarton = Number(product.cubic_feet ?? product.carton_cuft_final ?? product.carton_cuft_est ?? 0) || 0;
      const cappedCuft = capCartonByCategory(rawCarton, {
        title: product.name || product.title,
        category: product.category || (product.breadcrumbs || []).join(" > "),
        brand: (product.brand && (product.brand.name || product.brand)) || ""
      });
      const cubicFeetForFreight = cappedCuft;

      // Landed base: prefer existing field; else derive basic landed = price + duty
      const price = Number(product.price ?? itemPrice ?? 0) || 0;
      const dutyPct = Number(product.dutyPct ?? 0) || 0;
      const landedCandidate = Number(product.landed_cost ?? product.landed ?? 0);
      const derivedLanded = dutyPct ? price * (1 + dutyPct / 100) : price;
      const landedBase = isNaN(landedCandidate) || landedCandidate <= 0 ? derivedLanded : landedCandidate;

      // Distinct vendors (fallback 1)
      const vendors =
        Number(product.distinctVendors ?? product.vendorsCount ?? 1) || 1;

      const sh = computeShippingAndHandling({
        cubicFeet: cubicFeetForFreight,
        landed: landedBase,
        vendors,
      });

      // Clamp S&H by price multiplier
      if (sh && typeof sh.total_usd === "number") {
        const clamped = clampByMultiplier(sh.total_usd, price, { minMult:1.7, maxMult:2.5, fallbackMult:1.95 });
        if (clamped !== sh.total_usd) {
          sh.total_usd = Number(clamped.toFixed(2));
          sh._note = "clamped_by_multiplier";
        }
      }

      product.shipping_handling = sh;
      product.cubic_feet = Number(cappedCuft.toFixed(2));
      console.log(`📦 FREIGHT CARTON USED: ${product.cubic_feet} ft³ (capped from ${rawCarton})`);
      // keep backward compat if other code expects this field:
      product.shipping_cost = sh.total_usd;
    }
  } catch (e) {
  ...
  try {
    ...
    const applyNJTax = (process.env.APPLY_NJ_TAX || 'true') === 'true';
    const njTaxRate = Number(process.env.NJ_TAX_RATE_PCT || 6.625);
    const marginPct = Number(process.env.MARGIN_PCT || 20);
    const applyRound95 = (process.env.ROUND_RETAIL_95 || 'true') === 'true';

    const detectProfileSimple = (title, cat) => {
      const text = `${title} ${cat}`.toLowerCase();
      if (/\b(sofa|loveseat|sectional|couch)\b/.test(text)) return 'sofa';
      if (/\b(chair|armchair|recliner)\b/.test(text)) return 'chair';
      if (/\b(table|desk)\b/.test(text)) return 'table';
      if (/\b(bed|mattress)\b/.test(text)) return 'bed';
      return 'default';
    };

    const pricingResult = computePricing({
      itemPrice: product.price || 0,
      cubicFeet: product.cubic_feet || 0,
      ratePerFt3: oceanRate,
      minFreightUSD: minFreight,
      applyNJTax,
      njTaxRatePct: njTaxRate,
      marginPct,
      dutyRatePct: product.dutyPct,
      profile: detectProfileSimple(product.name || product.title, product.category || ''),
      applyRound95
    });

    product.pricing_v3 = pricingResult;
    console.log(`💰 Pricing v3: retail $${pricingResult.totals.retail} (landed: $${pricingResult.totals.landed})`);
  } catch (e) {
  ...
  const est = { cubic_feet: Number(product.cubic_feet || 0) };
  const priceUSD = Number(product.price || 0);
  const freightResult = computeFreight({
    cubicFeet: est.cubic_feet,
    priceUSD,
    fallbackPctOfPrice: 0.5
  });
  product.freight = {
    amount: freightResult.freight,
    inputs: freightResult.inputs
  };
  console.log(`🧮 Freight ${freightResult.freight} | ft³ ${est.cubic_feet || 0} | src ${product.dimension_source || 'n/a'} | mode ${freightResult.inputs.mode}`);
...
async function fastCalculate(req) {
  ...
  // Compute freight (synchronous, fast)
  const priceUSD = Number(price || 0);
  const fr = computeFreight({
    cubicFeet: Number(est.cubic_feet || 0),
    priceUSD,
    fallbackPctOfPrice: 0.5
  });

  console.log(`🧮 Freight ${fr.freight} | ft³ ${est.cubic_feet || 0} | src ${est.source || 'n/a'} | mode ${fr.inputs.mode}`);
  ...
  try {
    ...
    products.forEach(product => {
      if (product.price && product.price > 0) {
        ...
        const dutyPct = Number(product.dutyPct) || 0;
        const dutyAmount = itemPrice * (dutyPct / 100);
        const shippingCost = Number(product.shippingCost) || Number(product.shipping_cost) || 0;
        const marginPct = Number(process.env.MARGIN_PCT || 20);

        // Total landed cost = item price + duty + shipping + margin
        const subtotal = itemPrice + dutyAmount + shippingCost;
        ...
        lineItems.push({
          ...
          properties: [
            ...
            { name: 'Duty', value: `$${dutyAmount.toFixed(2)} (${dutyPct}%)` },
            { name: 'Shipping', value: `$${shippingCost.toFixed(2)}` },
