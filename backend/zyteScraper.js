...
class ZyteScraper {
  ...
  parseZyteResponse(data, url, retailer) {
    ...
    if (data.product) {
      ...
      // Extract variant options with prices for selector UI
      productData.variantOptions = this.extractVariantOptions(product, data.browserHtml, url);
  ...
  // Extract variant options with prices for selector UI
  extractVariantOptions(product, browserHtml = null, url = '') {
    console.log('   🎨 Extracting variant options with prices...');
    ...
    // First, try to extract from Zyte's product.variants array
    if (product.variants && Array.isArray(product.variants)) {
      console.log(`   📊 Found ${product.variants.length} variant options in Zyte array`);
      ...
      product.variants.forEach((variant, index) => {
        ...
        if (variant.color) {
          ...
          if (colorValue && !variantOptions.colors.find(c => c.value === colorValue)) {
            variantOptions.colors.push({ value: colorValue, price, url: variantUrl, availability });
          }
        }

        if (variant.size) {
          const sizeValue = this.cleanVariantValue(variant.size);
          if (sizeValue && !variantOptions.sizes.find(s => s.value === sizeValue)) {
            variantOptions.sizes.push({ value: sizeValue, price, url: variantUrl, availability });
          }
        }

        if (variant.style) {
          const styleValue = this.cleanVariantValue(variant.style);
          if (styleValue && !variantOptions.styles.find(s => s.value === styleValue)) {
            variantOptions.styles.push({ value: styleValue, price, url: variantUrl, availability });
          }
        }

        if (variant.material) {
          const materialValue = this.cleanVariantValue(variant.material);
          if (materialValue && !variantOptions.materials.find(m => m.value === materialValue)) {
            variantOptions.materials.push({ value: materialValue, price, url: variantUrl, availability });
          }
        }
      });
    }

    // For Pottery Barn family, parse HTML to get all variants
    const isPotteryBarnFamily = url && (
      url.includes('potterybarn.com') ||
      url.includes('westelm.com') ||
      url.includes('williams-sonoma.com') ||
      url.includes('potterybarnkids.com')
    );

    if (isPotteryBarnFamily && browserHtml) {
      console.log('   🏺 Pottery Barn family detected - parsing HTML for all variants...');
      const htmlVariants = this.extractPotteryBarnVariantsFromHTML(browserHtml);

      if (htmlVariants.sizes.length > 0) {
        variantOptions.sizes = htmlVariants.sizes;
        console.log(`   ✅ Found ${htmlVariants.sizes.length} size options from HTML`);
      }

      if (htmlVariants.colors.length > 0) {
        variantOptions.colors = htmlVariants.colors;
        console.log(`   ✅ Found ${htmlVariants.colors.length} color options from HTML`);
      }
    }

    console.log('   📦 Final variant options:', {
      sizes: variantOptions.sizes.length,
      colors: variantOptions.colors.length,
      styles: variantOptions.styles.length,
      materials: variantOptions.materials.length
    });

    return variantOptions;
  }

  // Extract Pottery Barn variants from HTML
  extractPotteryBarnVariantsFromHTML(html) {
    const variants = {
      sizes: [],
      colors: []
    };
    ...
    try {
      // Strategy 1: Look for embedded product JSON data (window.__INITIAL_STATE__ or similar)
      ...
      while ((scriptMatch = scriptRegex.exec(html)) !== null) {
        try {
          ...
          if (jsonData.offers && Array.isArray(jsonData.offers)) {
            jsonData.offers.forEach(offer => {
              if (offer.name && offer.price) {
                const sizeMatch = offer.name.match(/(Twin|Full|Queen|King|California King|Full\/Queen)/i);
                if (sizeMatch) {
                  const size = sizeMatch[1].trim();
                  if (!variants.sizes.find(s => s.value === size)) {
                    variants.sizes.push({
                      value: size,
      ...
      if (variants.sizes.length > 0) {
        console.log(`   🔍 Extracted ${variants.sizes.length} sizes from structured data`);
      }

      // Strategy 3: Look for size/price patterns in data attributes or text
      if (variants.sizes.length === 0) {
        const commonSizes = ['Twin', 'Full', 'Queen', 'King', 'California King', 'Full/Queen'];
        const priceRegex = /\$(\d+(?:\.\d{2})?)/g;

        commonSizes.forEach(size => {
          const sizePattern = new RegExp(`${size}[^$]*\\$(\\d+(?:\\.\\d{2})?)`, 'i');
        ...
        console.log(`   🔍 Extracted ${variants.sizes.length} sizes from pattern matching`);
      }

      // For debugging: if no variants found, show we tried
      if (variants.sizes.length === 0) {
        console.log('   ⚠️ No size variants found in HTML - customer will need to manually verify');
      }

      // Look for color options (lower priority)
      ...
      if (variants.colors.length > 0) {
        console.log(`   🔍 Extracted ${variants.colors.length} colors from HTML`);
      }

    } catch (error) {
      console.log('   ⚠️ Error extracting Pottery Barn variants:', error.message);
    }
